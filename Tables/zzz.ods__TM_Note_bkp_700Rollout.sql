...
[InsertDate] [datetime] NULL CONSTRAINT [DF__TM_Note__InsertD__17F790F9] DEFAULT (getdate()),
[UpdateDate] [datetime] NULL CONSTRAINT [DF__TM_Note__UpdateD__18EBB532] DEFAULT (getdate()),
...
GO
ALTER TABLE [zzz].[ods__TM_Note_bkp_700Rollout] ADD CONSTRAINT [PK__TM_Note__3213E83FD32E594C] PRIMARY KEY CLUSTERED  ([id]) WITH (DATA_COMPRESSION = PAGE)
GO
CREATE NONCLUSTERED INDEX [IX_TM_Note_note_id_task_stage_seq_num] ON [zzz].[ods__TM_Note_bkp_700Rollout] ([note_id], [task_stage_seq_num]) WITH (DATA_COMPRESSION = PAGE)
GO
