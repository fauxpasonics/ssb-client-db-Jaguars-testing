...
, ETL_FileName, ProspectID, PersonID, FootballPriority, FootballPriorityDate, TicketingSystemAccountID, AbilitecID
INTO #SrcData
FROM (select ETL_FileName, ProspectID, PersonID, FootballPriority, FootballPriorityDate, TicketingSystemAccountID, AbilitecID,
	ROW_NUMBER() OVER(Partition By ProspectID, PersonID, TicketingSystemAccountID Order By FootballPriorityDate DESC) RowNum
	from stg.Turnkey_Models) A
WHERE RowNum = 1
;
...
UPDATE #SrcData
SET ETL_DeltaHashKey = HASHBYTES('sha2_256', ISNULL(RTRIM(AbilitecID),'DBNULL_TEXT') + ISNULL(RTRIM(FootballPriority),'DBNULL_TEXT') + ISNULL(RTRIM(FootballPriorityDate),'DBNULL_TEXT') + ISNULL(RTRIM(PersonID),'DBNULL_TEXT') + ISNULL(RTRIM(ProspectID),'DBNULL_TEXT') + ISNULL(RTRIM(TicketingSystemAccountID),'DBNULL_TEXT'))

EXEC etl.LogEventRecordDB @Batchid, 'Info', @ProcedureName, 'Merge Load', 'Src Table Setup', 'ETL_DeltaHashKey Set', @ExecutionId

CREATE NONCLUSTERED INDEX IDX_Load_Key ON #SrcData (PersonID)
...
ON myTarget.PersonID = mySource.PersonID AND myTarget.ProspectID = mySource.ProspectID AND myTarget.TicketingSystemAccountID = mySource.TicketingSystemAccountID
...
THEN UPDATE SET
	 ...
	 , myTarget.[AbilitecID] = mySource.[AbilitecID]
