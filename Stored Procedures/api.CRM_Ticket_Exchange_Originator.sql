...
END
		...
		INTO #tmpBase
        FROM    ods.TM_Tex Tex
                INNER JOIN dbo.DimCustomer DimCustomer WITH ( NOLOCK ) ON DimCustomer.AccountId = Tex.owner_acct_id AND DimCustomer.CustomerType = 'Primary' AND DimCustomer.SourceSystem = 'TM'
        ...
        WHERE   Tex.activity_name = 'TE Resale'
			...
			AND tex.event_date >= DATEADD(YEAR, -2, GETDATE())

-- Pull total count
SELECT @totalCount = COUNT(*) FROM #tmpBase

-- Load base data
SELECT    ISNULL(O_Activity_Name,'') Activity_Name
