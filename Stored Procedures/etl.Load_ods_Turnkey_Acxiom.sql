...
FROM (
	...
	, ROW_NUMBER() OVER(PARTITION BY AbilitecID, ProspectID ORDER BY ETL_ID) RowRank
...
CREATE NONCLUSTERED INDEX IDX_Load_Key ON #SrcData (AbilitecID, ProspectID)
...
ON myTarget.AbilitecID = mySource.AbilitecID and myTarget.ProspectID = mySource.ProspectID
...
--WHEN NOT MATCHED BY SOURCE THEN UPDATE SET myTarget.ETL_IsDeleted = 1, myTarget.ETL_DeletedDate = @RunTime
