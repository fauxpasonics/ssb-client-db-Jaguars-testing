...
THEN UPDATE SET 
	...
	--20180404 jbarberio: Soft Delete table to track merges provided by Jaguars
	UPDATE dc
	SET dc.IsDeleted = 1
		,dc.DeleteDate = GETDATE()
	FROM dbo.DimCustomer dc
		JOIN [dbo].[TM_SoftDeletes] del ON del.Accountid = dc.AccountId
	WHERE dc.SourceSystem = 'tm'
		  AND dc.IsDeleted = 0

	--set matchkeys
	UPDATE a
	SET customer_matchkey = 'TM-' + CAST(a.AccountId AS NVARCHAR(100))
	   ,a.matchkey_updatedate = GETDATE()
	FROM dbo.dimcustomer a 
	WHERE a.SourceSystem = 'TM'
		  AND accountid IS NOT NULL
		  AND accountid != '-1'
		  AND a.IsDeleted = 0
		  AND a.customer_matchkey IS NULL
