...
CREATE VIEW [etl].[vw_EloquaOutbound_DynamicsContact_DEV]
...
FROM (
		SELECT	 EloquaContact.EmailAddress											AS emailAddress		
				...
				,CRMContact.str_number												AS jaguarsId
		...
		FROM ods.Eloqua_Contact EloquaContact (NOLOCK)
			JOIN dbo.dimcustomerssbid eloquaSSBID (NOLOCK) ON eloquaSSBID.ssid = CAST(eloquacontact.ContactIDExt	 AS NVARCHAR(100))
			JOIN Jaguars_Reporting.prodcopy.Contact CRMContact (NOLOCK) ON CRMContact.emailaddress1 = EloquaContact.EmailAddress
			LEFT JOIN (SELECT SSB_CRMSYSTEM_CONTACT_ID
					   ...
					   FROM ods.TM_PayScheduleMOP tm (NOLOCK)
						JOIN dbo.DimCustomer dc (NOLOCK) ON dc.AccountId = tm.acct_id
			...
			LEFT JOIN  (SELECT x.entityid
						...
						FROM ( SELECT entityid
							   ...
							   FROM prodcopy.List list (NOLOCK)
	   							JOIN prodcopy.ListMember (NOLOCK) ON ListMember.listid = list.listid
