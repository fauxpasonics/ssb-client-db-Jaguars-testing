...
-- 7/18/2018: Jeff Barberio
	...
	-- Change notes: Add accountid mapping to ExtAttribute4 to be used in an account level matchkey to maintain account/contact relationships
...
SELECT  DB_NAME() AS [SourceDB]
      , 'CRM_Accounts' AS [SourceSystem]
      , 2 AS [SourceSystemPriority]


/*Standard Attributes*/
      , CAST(a2.accountid AS NVARCHAR(100)) AS [SSID]
      , a2.CustomerTypeCode AS [CustomerType]
      , NULL AS [CustomerStatus]
      , NULL AS [AccountType]
      , a2.str_partnersalesperson AS [AccountRep]
...
/*AddressPrimary*/
      , ISNULL(a2.address1_line1,'') + ' ' + ISNULL(a2.address1_line2,'') + ' ' + ISNULL(a2.address1_line3,'') AS [AddressPrimaryStreet]
...
/*Phone*/
      , NULLIF(LEFT(a2.telephone1,25),'') AS [PhonePrimary]
      , NULL AS [PhoneHome]
      , NULL AS [PhoneCell]
      , NULLIF(LEFT(a2.telephone2,25),'')  AS [PhoneBusiness]
      , NULLIF(LEFT(a2.Fax,25),'')		   AS [PhoneFax]
      , NULL AS [PhoneOther]

/*Email*/
      , NULLIF(a2.emailaddress1,'') AS [EmailPrimary]
      , NULLIF(a2.emailaddress2,'') AS [EmailOne]
      , NULLIF(a2.emailaddress3,'') AS [EmailTwo]

/*Extended Attributes*/
      , a2.donotphone AS [ExtAttribute1] --nvarchar(100)
      , NULL AS [ExtAttribute2]
      , NULL AS [ExtAttribute3]
      , CAST(a2.accountid AS NVARCHAR(100)) AS [ExtAttribute4]
      , NULL AS [ExtAttribute5]
      , NULL AS [ExtAttribute6]
      , NULL AS [ExtAttribute7]
      , NULL AS [ExtAttribute8]
      , NULL AS [ExtAttribute9]
      --, a2.str_number AS [ExtAttribute10]
...
/*Source Created and Updated*/
      ...
      , a2.createdon [SSCreatedDate]
      , a2.modifiedon [SSUpdatedDate]
      , GETDATE() CreatedDate
      , GETDATE() UpdatedDate
      , NULL [AccountId]
      , 1 IsBusiness
	  , statecode IsDeleted
FROM   Jaguars_Reporting.prodcopy.account AS a2 WITH ( NOLOCK )
WHERE 1 = 1
AND a2.CopyLoadDate > DATEADD(DAY,-3,GETDATE())
                ) a
);
