...
CREATE VIEW [ods].[vw_TM_LoadDimCustomer_MANUAL] AS (
...
--base set
		...
		FROM ods.TM_Cust cust	
			JOIN (SELECT SSID
				  FROM mdm.tmp_MDM_CombinedNames cn
					JOIN dimcustomer dc ON dc.DimCustomerId = cn.DimCustomerId
				  )cn ON cn.SSID = CONCAT(cust.acct_id,':',cust.cust_name_id)
		WHERE 1=1
