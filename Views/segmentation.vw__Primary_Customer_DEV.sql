...
CREATE VIEW  [segmentation].[vw__Primary_Customer_DEV]
AS
    ( 
	SELECT    ssbid.SSB_CRMSYSTEM_CONTACT_ID
              ...
              , NULLIF(TRY_CAST(CASE WHEN dimcustomer.PhonePrimaryIsCleanStatus = 'Valid'
                          THEN LEFT(REPLACE(REPLACE(REPLACE(REPLACE(dimcustomer.PhonePrimary ,' ',''),'-',''),')',''),'(',''),3)
                          ELSE NULL
                     END AS INT),0) AS Phone_Primary_Area_Code
              , dimcustomer.PhonePrimaryIsCleanStatus AS Phone_Primary_Is_Clean_Status
			  , dimcustomer.PhoneHome AS Phone_Home
              , NULLIF(TRY_CAST(CASE WHEN dimcustomer.PhoneHomeIsCleanStatus = 'Valid'
                          THEN LEFT(REPLACE(REPLACE(REPLACE(REPLACE(dimcustomer.PhoneHome ,' ',''),'-',''),')',''),'(',''),3)
                          ELSE NULL
                     END AS INT),0) AS PhoneHomeAreaCode
              , dimcustomer.PhoneHomeIsCleanStatus AS Phone_Home_Is_Clean_Status
			  , dimcustomer.PhoneCell AS Phone_Cell
              , NULLIF(TRY_CAST(CASE WHEN dimcustomer.PhoneCellIsCleanStatus = 'Valid'
                          THEN LEFT(REPLACE(REPLACE(REPLACE(REPLACE(dimcustomer.PhoneCell ,' ',''),'-',''),')',''),'(',''),3)
                          ELSE NULL
                     END AS INT),0) AS Phone_Cell_Area_Code
              , dimcustomer.PhoneCellIsCleanStatus AS Phone_Cell_Is_Clean_Status
			  , dimcustomer.PhoneBusiness AS PhoneBusiness
              , NULLIF(TRY_CAST(CASE WHEN dimcustomer.PhoneBusinessIsCleanStatus = 'Valid'
                          THEN LEFT(REPLACE(REPLACE(REPLACE(REPLACE(dimcustomer.PhoneBusiness ,' ',''),'-',''),')',''),'(',''),3)
                          ELSE NULL
                     END AS INT),0) AS Phone_Business_Area_Code
              , dimcustomer.PhoneBusinessIsCleanStatus AS Phone_Business_Is_Clean_Status
			  , dimcustomer.PhoneFax AS Phone_Fax
              , NULLIF(TRY_CAST(CASE WHEN dimcustomer.PhoneFaxIsCleanStatus = 'Valid'
                          THEN LEFT(REPLACE(REPLACE(REPLACE(REPLACE(dimcustomer.PhoneFax ,' ',''),'-',''),')',''),'(',''),3)
                          ELSE NULL
                     END AS INT),0) AS Phone_Fax_Area_Code
              , dimcustomer.PhoneFaxIsCleanStatus AS Phone_Fax_Is_Clean_Status
			  , dimcustomer.PhoneOther AS Phone_Other
              , NULLIF(TRY_CAST(CASE WHEN dimcustomer.PhoneOtherIsCleanStatus = 'Valid'
                          THEN LEFT(REPLACE(REPLACE(REPLACE(REPLACE(dimcustomer.PhoneOther ,' ',''),'-',''),')',''),'(',''),3)
                          ELSE NULL
                     END AS INT),0) AS Phone_Other_Area_Code
