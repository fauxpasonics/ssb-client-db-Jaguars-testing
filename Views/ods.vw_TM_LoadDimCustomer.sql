...
--base set
		SELECT 
			...
			/*Phone*/
			, NULLIF(LEFT(phone_day,25),'') AS [PhonePrimary]
			, NULLIF(LEFT(phone_eve,25),'') AS [PhoneHome]
			, NULLIF(LEFT(phone_cell,25),'') AS [PhoneCell]
			, NULL AS [PhoneBusiness]
			, NULLIF(LEFT(phone_fax,25),'') AS [PhoneFax]
			, NULL AS [PhoneOther]
			...
			, 'TM-' + CAST(acct_id AS NVARCHAR(25)) + ':' + CAST(cust_name_id AS NVARCHAR(25)) AS[ExtAttribute5] 
			...
			, 0 AS IsDeleted
			, CASE WHEN cust.Primary_code = 'primary' AND NULLIF(cust.acct_id,'') IS NOT NULL THEN CONCAT('TM-',cust.acct_id) ELSE NULL END customer_matchkey
		FROM ods.TM_Cust cust	
			LEFT JOIN [dbo].[TM_SoftDeletes] del ON del.accountid = cust.acct_id
		WHERE 1=1
		AND updatedate > (GETDATE() - 3)
		AND del.accountid IS NULL
