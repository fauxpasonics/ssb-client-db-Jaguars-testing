...
CREATE VIEW [etl].[vw_Load_DimCustomer_Turnkey_MANUAL] AS 
...
*/
		...
		FROM [ods].[Turnkey_Acxiom] p		
			JOIN (SELECT ssid
				  FROM mdm.tmp_MDM_CombinedNames_Turnkey cn
					JOIN dbo.DimCustomer dc ON dc.DimCustomerId = cn.DimCustomerId
				  )cn ON cn.ssid = CAST(CONCAT(AbilitecID,':',ProspectID) AS NVARCHAR(100))			
		WHERE 1=1
