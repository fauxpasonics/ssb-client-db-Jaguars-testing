...
-- Created date: 1/28/2018

-- Purpose: Dimcustomer Load view 
...
-- 7/18/2018: Jeff Barberio

	-- Change notes: Add parentcustomerid mapping to ExtAttribute4 to be used in an account level matchkey to maintain account/contact relationships
...
CREATE VIEW [etl].[vw_Load_DimCustomer_DynamicsContacts] AS 
...
SELECT  DB_NAME() AS [SourceDB]
      , 'CRM_Contacts' AS [SourceSystem]
      , NULL AS [SourceSystemPriority]


/*Standard Attributes*/
      , CAST(c.contactid AS NVARCHAR(100)) AS [SSID]
      , NULL AS [CustomerType]
      , NULL AS [CustomerStatus]
      , NULL AS [AccountType]
      , c.ownerid AS [AccountRep]
      , c.company AS [CompanyName]
      , c.salutation AS [SalutationName]
...
/*AddressPrimary*/
      , c.address1_line1 AS [AddressPrimaryStreet]
...
/*Phone*/
      , CAST(NULLIF(c.telephone1,'') AS NVARCHAR(25)) AS [PhonePrimary]
      , CAST(NULL AS NVARCHAR(25)) AS [PhoneHome]
      , CAST(NULLIF(c.mobilephone,'') AS NVARCHAR(25)) AS [PhoneCell]
      , CAST(NULLIF(c.telephone2,'') AS NVARCHAR(25)) AS [PhoneBusiness]
      , CAST(NULLIF(c.fax,'') AS NVARCHAR(25)) AS [PhoneFax]
      , NULL AS [PhoneOther]

/*Email*/
      , NULLIF(c.emailaddress1,'') AS [EmailPrimary]
      , NULLIF(c.emailaddress2,'') AS [EmailOne]
      , NULLIF(c.emailaddress3,'') AS [EmailTwo]

/*Extended Attributes*/
      , c.donotphone AS [ExtAttribute1]
	  , CAST(str_number AS NVARCHAR(100)) AS [ExtAttribute2] --nvarchar(100)
      , c.contactid AS [ExtAttribute3]
      , CAST(c.parentcustomerid AS NVARCHAR(100)) AS [ExtAttribute4]
      , CASE WHEN ISNULL(c.new_ssbcrmsystemssidwinnersourcesystem,'') = '' OR ISNULL(c.new_ssbcrmsystemssidwinner,'') = '' THEN NULL
			 WHEN c.new_ssbcrmsystemssidwinnersourcesystem = 'TM' THEN 'TM-' + c.new_ssbcrmsystemssidwinner
			 WHEN c.new_ssbcrmsystemssidwinnersourcesystem = 'Eloqua' THEN 'ELO-' + c.new_ssbcrmsystemssidwinner
			 WHEN c.new_ssbcrmsystemssidwinnersourcesystem = 'TUrnkey' THEN 'TURN-' + c.new_ssbcrmsystemssidwinner
			 ELSE NULL END
			 AS [ExtAttribute5]
...
/*Source Created and Updated*/
      , c.createdby [SSCreatedBy]
      , c.modifiedby [SSUpdatedBy]
      , c.createdon [SSCreatedDate]
      , c.modifiedon [SSUpdatedDate]
      , GETDATE() CreatedDate
      , GETDATE() UpdatedDate
	  ,NULL AS [AccountId]
      , 0 IsBusiness
	  , statecode IsDeleted
	  , CASE WHEN (ISNULL(c.new_ssbcrmsystemssidwinner,'') = '' OR ISNULL(c.new_ssbcrmsystemssidwinnersourcesystem,'') = '') AND  CHARINDEX('|',str_number,1) > 0 AND c.str_number != '' THEN   'TM-' + SUBSTRING(c.str_number,1,CHARINDEX('|',str_number,1)-1)
			WHEN (ISNULL(c.new_ssbcrmsystemssidwinner,'') = '' OR ISNULL(c.new_ssbcrmsystemssidwinnersourcesystem,'') = '') AND CHARINDEX('|',str_number,1) = 0 THEN 'TM-' + str_number
			ELSE NULL 
	  END AS customer_matchkey
FROM    Jaguars_Reporting.prodcopy.Contact AS c WITH ( NOLOCK )

WHERE 1 = 1
AND c.CopyLoadDate > DATEADD(DAY,-3,GETDATE())
