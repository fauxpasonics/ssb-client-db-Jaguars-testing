...
*/
		...
		SELECT 
			'Jaguars' AS [SourceDB]
			, 'Turnkey' AS [SourceSystem]
			, NULL AS [SourceSystemPriority]

			/*Standard Attributes*/
			, CAST(CONCAT(AbilitecID,':',ProspectID) AS NVARCHAR(100)) [SSID]
			...
			/**ENTITIES**/
			/*Name*/			
			, NULLIF(LEFT(FirstName,200),'') AS FirstName
			, NULL AS [Prefix]
			, NULL  AS FullName
		    , NULLIF(LEFT(LastName,200),'') AS [LastName]
			, NULL AS [MiddleName]
			, NULL AS [Suffix]
			...
			, CAST(NULLIF(HomePhone,'') AS NVARCHAR(25)) AS  PhonePrimary
			, CAST(NULLIF(HomePhone,'') AS NVARCHAR(25)) AS [PhoneHome]
			, CAST(NULLIF(MobilePhone,'') AS NVARCHAR(25)) AS [PhoneCell]
			, NULL AS [PhoneBusiness]
			, NULL AS [PhoneFax]
			, NULL AS [PhoneOther]

			/*Email*/
			, CAST(NULLIF(Email,'') AS NVARCHAR(256)) AS [EmailPrimary]
			...
			, 'TURN-'+CAST(CONCAT(AbilitecID,':',ProspectID) AS NVARCHAR(100)) AS[ExtAttribute5] 
			...
			, CONVERT(NVARCHAR(255),NULL) [SSCreatedBy]
			, CONVERT(NVARCHAR(255),NULL) [SSUpdatedBy]
			, CONVERT(DATETIME,TRY_CAST(AbilitecIDAppendDate AS DATETIME)) [SSCreatedDate]
			, CONVERT(DATETIME,NULL) [SSUpdatedDate]
			, GETDATE() CreatedDate
			, GETDATE() UpdatedDate
			, NULL [AccountId]
			, '0' IsBusiness
